...
def main():
    ...
    if main_partition_name != SD_CARD_PARTITION_NAME:
        module.exit_json(changed=False, stderr="WARN: The root partition {} does not appear to be on an SD card, resize via raspi_config will not work. Doing nothing.".format(main_partition_name))
    else:
        (rc, out, err) = module.run_command(["sudo", "raspi_config", "--expand-rootfs"], check_rc=True)
